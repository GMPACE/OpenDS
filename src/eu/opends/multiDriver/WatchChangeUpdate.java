...
public class WatchChangeUpdate implements Update {
	...
	public WatchChangeUpdate(Simulator sim2, String vehicleID,
			String positionString, String rotationString, String headingString,
			String wheelString) {
		this.sim = sim2;
	...
	public void performUpdate() {
		// System.err.println("changeVehicle() --> vehicleID: " + vehicleID +
		// ", positionString: " + positionString + " , rotationString: " +
		// rotationString + " , wheelString: " + wheelString);
		//if (vehicleID.equals(sim.getMultiDriverClient().getWatchCar())) {
			...
			if (car != null) {
				...
				if (arrayWheel.length == 2) {
					...
					car.steer(steering);
					car.setSteeringWheelState(wheelPosition);
		...
		//} else {
			...
			if (arrayPos.length == 3) {
				...
				double yPos = Double.parseDouble(arrayPos[1]);
				double zPos = Double.parseDouble(arrayPos[2]);
				// position = GeoPosition.geoToModel(new Vector3d(xPos, yPos,
				// zPos));
				position = (new Vector3d(xPos, yPos, zPos)).toVector3f();
	...
	private Quaternion adjustRotation(Vector3f position, float yaw) {
		if(sim.getSceneNode() == null) return null;
		...
		Vector3f frontContactPoint = castPerpendicularRay(frontPoint
				.getWorldTranslation());

		Node backPoint = (Node) carNode.getChild("backPoint");
		Vector3f backContactPoint = castPerpendicularRay(backPoint
				.getWorldTranslation());

		if (frontContactPoint != null && backContactPoint != null) {
			float heightDiff = backContactPoint.getY()
					- frontContactPoint.getY();
			float zDistance = backPoint.getLocalTranslation().getZ()
					- frontPoint.getLocalTranslation().getZ();

			pitch = -FastMath.atan(heightDiff / zDistance); // pos pitch: front
															// up / neg pitch:
															// back up
		}
		...
		Vector3f leftContactPoint = castPerpendicularRay(leftPoint
				.getWorldTranslation());

		Node rightPoint = (Node) carNode.getChild("rightPoint");
		Vector3f rightContactPoint = castPerpendicularRay(rightPoint
				.getWorldTranslation());

		if (leftContactPoint != null && rightContactPoint != null) {
			float heightDiff = rightContactPoint.getY()
					- leftContactPoint.getY();
			float xDistance = rightPoint.getLocalTranslation().getX()
					- leftPoint.getLocalTranslation().getX();

			roll = FastMath.atan(heightDiff / xDistance); // pos roll: right up
															// / neg roll: left
															// up
		}
	...
	private Vector3f castPerpendicularRay(Vector3f origin) {
		...
		/*
		 * // use the results if (results.size() > 0) { // the closest collision
		 * point is what was truly hit return
	...
	private boolean isGeometryOfMultiDriverVehicle(Geometry geometry) {
		...
		if (sim.getMultiDriverClient() != null)
			registeredVehicles = sim.getMultiDriverClient()
	...
	private void setPosition(String vehicleID, Vector3f position) {
		try {
			...
			System.err.println("Could not set position of vehicle '"
					+ vehicleID + "'!");
		}
	}
	...
	private void setRotation(String vehicleID, Quaternion rotation) {
		try {
			...
			System.err.println("Could not set rotation of vehicle '"
					+ vehicleID + "'!");
		}
	}
	...
	private void setWheels(String vehicleID, float steering, float position) {
		try {
			...
			// System.out.println("steering: " + steering + " - position: " +
			// position);
			...
			/*
			 * // TODO does not work !!!!
			 * control.getWheel(0).getWheelSpatial().setLocalRotation(new
			 * Quaternion().fromAngles(position, steering, 0));
			 * control.getWheel(
			 * 0).getWheelSpatial().getWorldRotation().fromAngles(position,
			 * steering, 0);
			 * control.getWheel(1).getWheelSpatial().rotate(position, steering,
			 * 0); control.getWheel(2).getWheelSpatial().rotate(position, 0, 0);
