...
public class TrafficCar extends Car implements TrafficObject {
	...
	public TrafficCar(Simulator sim, TrafficCarData trafficCarData) {
		...
		// initial position and rotation not needed, as car will automatically
		// be
		...
		/*
		 * //--------------------------------- // add bounding sphere to a
		 * traffic car which can be hit by the user-controlled car Sphere sphere
		 * = new Sphere(20, 20, 2.5f); Geometry boundingSphere = new
		 * Geometry(name + "_boundingSphere", sphere); Material
		 * boundingSphereMaterial = new Material(sim.getAssetManager(),
	...
	private void updateSpeed(ArrayList<TrafficObject> vehicleList) {
		...
		// stop car in order to avoid collision with other traffic objects and
		// driving car
		...
		// System.out.print(" *** " + gasPedalPressIntensity *
		// accelerationForce);
	...
	/**
	 * Returns the signum of the speed change between this and the previous way
	 * point: 0 if speed has not changed (or no previous way point available), 1
	 * if speed has been increased, -1 if speed has been decreased.
	 * 
	 * @return The signum of the speed change between this and the previous way
	 *         point
	 */
	...
	// 여기가 다른 트래픽의 자동차간의 거리를 나타내는 부분(도로정보 상의 자동차)
	...
	private boolean obstacleTooClose(Vector3f obstaclePos) {
		...
		if (nextWP != null) {
			// angle between direction towards next WP and direction towards
			// obstacle
			// (consider 3D space, because obstacle could be located on a bridge
			// above traffic car)
	...
	private boolean belowSafetyDistance(float angle, float distance) {
		...
		if (useSpeedDependentForwardSafetyDistance)
			...
			//speedDependentForwardSafetyDistance = 0;
