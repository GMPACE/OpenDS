...
public class FollowBox {
	...
	public FollowBox(Simulator sim, final TrafficObject trafficObject, FollowBoxSettings settings) {
		...
		motionPath.setPathSplineType(SplineType.CatmullRom); // --> default:
																// CatmullRom
	...
	public void update(Vector3f trafficObjectPos) {
		...
		// if new WP to set traffic object available, wait for NEXT update and
		// set
		...
		checkIfWayPointReached(null);
	}

	public void update(Vector3f trafficObjectPos, String name) {
		// skip "else"-part during initialization (first 3 update loops)
		if (sim.isPause() || counter < 3) {
			motionControl.setSpeed(0f);
			counter++;
		...
		if (isTargetWayPointAvailable && (waitForNextUpdate = !waitForNextUpdate)) {
			// set traffic object to new position
			if(name == "mycar"){
				performWayPointChange(targetWayPointIndex, name);
				isTargetWayPointAvailable = false;
			}
		}

		checkIfWayPointReached(name);
	}
	
	private void checkIfWayPointReached(String name) {
		int currentWayPointIndex = motionControl.getCurrentWayPoint() % waypointList.size();
		
//		if(name != null && name == "mycar"){
//			System.out.println(name + " is " + currentWayPointIndex);
//		}
		...
		if (currentWayPointIndex != previousWayPointIndex) {
			...
			// if last WP reached and path not cyclic --> reset traffic object
			// to first WP
	...
	private boolean waitAtWP(int currentWayPointIndex) {
		...
		if (waitingTime == null || waitingTime <= 0) {
			...
			if (!isSetWaitTimer) {
				// waiting timer not yet set --> set timer to current time stamp
				// and wait
	...
	private void performWayPointChange(int index) {
		...
		// System.err.println("SET: dist " + traveledDistance + ", time: " +
		// traveledTime + ", index: " + index);
	...
	private void performWayPointChange(int index, String name) {
		// set follow box to WP
		float traveledDistance = 0;
		for (int i = 0; i < index; i++)
			traveledDistance += motionPath.getSpline().getSegmentsLength().get(i);
	...
	public float getHeadingAtWP(int index) {
		...
		if (nextWayPoint != null) {
			// compute driving direction by looking at next way point from
			// current position
			...
			// find the value from {heading1,heading2} which matches with one of
			// {heading3,heading4}
			...
			if ((diff_1_3 < diff_1_4 && diff_1_3 < diff_2_3 && diff_1_3 < diff_2_4)
				...
				// if diff_1_3 or diff_1_4 are smallest --> the correct heading
				// is heading1
				heading = heading1;
			} else {
				// if diff_2_3 or diff_2_4 are smallest --> the correct heading
				// is heading2
				heading = heading2;
			}
		}
	...
	public Waypoint getPreviousWayPoint(int index) {
		...
		if (motionPath.isCycle()) {
			// if path is cyclic, the predecessor of the first WP will be the
			// last WP
	...
	public Waypoint getNextWayPoint(int index) {
		...
		if (motionPath.isCycle()) {
			// if path is cyclic, the successor of the last WP will be the first
			// WP
	...
	private Spatial createFollowBox() {
		// add spatial representing the position the driving car is steering
		// towards
	...
	/*
	 * private boolean maxDistanceExceeded(Vector3f trafficObjectPos) { float
	 * currentDistance = getCurrentDistance(trafficObjectPos);
	 * 
	 * // report whether maximum distance is exceeded return currentDistance >
	 * maxDistance; }
	 */
	...
	public float getReducedSpeed() {
		...
		if (nextWP != null) {
			...
			float currentSpeed = currentSpeedInKmh / 1.6f;

			// speed at next way point
			float targetSpeedInKmh = nextWP.getSpeed();
			float targetSpeed = targetSpeedInKmh / 1.6f;

			// if speed at the next WP is lower than at the current WP --> brake
			// traffic object
			if (targetSpeed < currentSpeed) {
				...
				// speed difference in m/s between current WP's speed and next
				// WP's speed
				float speedDifference = currentSpeed - targetSpeed;

				// compute the distance in front of the next WP at what the
				// traffic object has to start
				// braking with 50% brake force in order to reach the next WP's
				// (lower) speed in time.
				float deceleration50Percent = 50f * trafficObject.getMaxBrakeForce() / trafficObject.getMass();
				
				float deceleration0Percent = 0f * trafficObject.getMaxBrakeForce() / trafficObject.getMass();

				// time in seconds needed for braking process
				float time = speedDifference / deceleration50Percent;
				//float time = speedDifference / deceleration0Percent;

				// distance covered during braking process
				float coveredDistance = 0.5f * -deceleration50Percent * time * time + currentSpeed * time;
				//float coveredDistance = 0.5f * -deceleration0Percent * time * time + currentSpeed * time;
				...
				if (distanceToBrakingPoint < 0) {
					...
					/*
					 * if(trafficObject.getName().equals("car1")) { float
					 * trafficObjectSpeedInKmh =
					 * trafficObject.getLinearSpeedInKmh();
					 * System.out.println(curentWP.getName() + " : " +
					 * speedPercentage + " : " + reducedSpeedInKmh + " : " +
					 * trafficObjectSpeedInKmh + " : " + targetSpeedInKmh); }
					 */
				}
			}
		}
